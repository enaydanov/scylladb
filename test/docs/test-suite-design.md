...
**Parameters:** `shortname: str`, `suite: TestSuite`

Sets the following instance state:

| Variable | Derivation |
|----------|------------|
| `id` | Generated by `suite.next_id((shortname, suite.suite_key))` |
...
4. Creates a `Test` instance (which internally generates a monotonic ID
   via `suite.next_id()`).
