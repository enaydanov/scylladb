...
class ScyllaClusterManager:
    ...
    Args:
        ...
        logger: logger for the manager and its cluster
    ...
    async def run_in_thread(self) -> AsyncIterator[ScyllaClusterManager]:
        ...
        with ThreadPoolExecutor(max_workers=1, thread_name_prefix="cluster-manager") as executor:
            ...
            try:
                ...
                stop_event.set()
                # Wait for the manager thread off the event loop, so the
                # caller's loop stays free to serve whatever stopping the
                # manager schedules on it.
