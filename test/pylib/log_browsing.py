...
class SyncScyllaLogFile:
    """Browse a Scylla log file (sync version).
        ...
        with self.file.open(encoding="utf-8") as log_file:
            ...
            while exprs:
                if time.perf_counter() > deadline:
                    raise TimeoutError(f"{exprs=} not found in {self.file.name} in {timeout} seconds")
