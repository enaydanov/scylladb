...
async def scylla_cluster(request: pytest.FixtureRequest,
    ...
    """
    async with testpy_cluster_factory(request.node, f"{testpy_uname}::{request.node.name}") as cluster:
        yield cluster
...
async def manager(request: pytest.FixtureRequest,
    ...
    async with ScyllaClusterManager(
            test_name=f"{testpy_uname}::{test_case_name}",
        ...
        # The failures detected here surface as teardown errors, too late for
        # pytest_runtest_makereport to keep the cluster: mark it directly.
        if cluster_status is not None and cluster_status["tasks_leaked"] and not failed:
            failed = True
            mgr.cluster.preserve_for_debugging = True
            pytest.fail(
